...
async def run() -> None:
    ...
    with mock.patch.object(cloudinary.uploader, "upload", side_effect=Exception("boom")):
        await assert_raises(StorageUploadError, provider.upload, b"x", "paper.pdf")
    ...
    with mock.patch.object(cloudinary.api, "delete_resources", side_effect=Exception("boom")):
        await assert_raises(StorageDeleteError, provider.delete, "paper.pdf")
    ...
    fake_response.content = b"pdf-bytes"
    client_mock = mock.AsyncMock()
    client_mock.get.return_value = fake_response
    client_mock.__aenter__.return_value = client_mock
    with (
        mock.patch.object(cloudinary.api, "resource", return_value=resource),
        mock.patch("httpx.AsyncClient", return_value=client_mock),
    ):
    ...
    with mock.patch.object(
        ...
        await assert_raises(StorageDownloadError, provider.download, "paper.pdf")
